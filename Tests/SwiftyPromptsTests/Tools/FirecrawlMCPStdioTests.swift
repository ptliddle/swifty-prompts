...
final class FirecrawlMCPStdioTests: XCTestCase {
    ...
    func testMCPStdioClient() async throws {
        ...
        let executableURL = URL(fileURLWithPath: npxPath)
        ...
        do {
            ...
            XCTFail("Error testing stdio client: \(error)")
        }
    }
    ...
    private func runCommand(_ arguments: [String]) async throws -> String? {
        var process = Process()
        let executablePath: String = "/usr/bin;usr/bin/env"
